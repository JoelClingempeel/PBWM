...
parser.add_argument('--num_gates', type=int, default=3)
...
parser.add_argument('--num_train', type=int, default=9001)
...
ACTIONS = [torch.tensor([action]).float()
           for action in get_actions(args['num_gates'])]


class PFC:
    def __init__(self, stripe_size, num_stripes):
    ...
    def update(self, data, gates):
        for index, gate in enumerate(gates.squeeze(0)):
            if gate.item():
                self.stripes[index] = data
            else:
                self.stripes[index] = torch.zeros(1, self.stripe_size)
...
class DQNSolver:
    def __init__(self, data_src, q_net, pfc, optimizer, num_symbols, num_gates,
...
dqn = nn.Sequential(
    nn.Linear(num_symbols + 3 + num_symbols * num_gates + num_gates, dqn_hidden_dim),
    nn.ReLU(),
    nn.Linear(dqn_hidden_dim, 1),
    nn.ReLU()
)
pfc = PFC(num_gates, num_symbols)
...
solver.train(1000)
solver.eps = 0
solver.train(args['num_train'] - 1000)
